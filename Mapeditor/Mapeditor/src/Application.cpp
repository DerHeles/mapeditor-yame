...
void Application::createMap()
{
	...
	if(m_map.load("resources/tilesets/" + tileset, sf::Vector2u(tile_width, tile_height), tiles, width, height, tile_gap, collisionTiles))
	{
		...
		m_gui.mapLoaded();
		m_gui.loadTiles("resources/tilesets/" + tileset, tile_width, tile_height, tile_gap);
	}
	else
	{
...
void Application::loadMap()
{
	...
	std::ifstream mapfile(std::string("resources/maps/") + mapname, std::ios::binary);
	...
	{
		std::cerr << "error loading map from \"" << "resources/maps/" << mapname << "\"" << std::endl;
	}
}
...
void Application::configMap()
{
	...
	if (m_map.reload("resources/tilesets/" + tileset, sf::Vector2u(tile_width, tile_height),  tile_gap))
	{
...
void Application::saveMap() const
{
	...
	std::ofstream mapfile(std::string("resources/maps/") + mapname, std::ios::binary);
